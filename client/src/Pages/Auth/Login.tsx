      ...
      <AuthLayout title="Welcome back" description="Login to your account to continue using KIKI">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
                ...
                <input type="email" id="email" placeholder="e.g johndoe@mail.com" className="input w-full" {...register("email")} />
                {errors.email && <p className="text-red-500 text-sm mt-1">{errors.email.message}</p>}
            </div>
            <div>
                <label htmlFor="password">Password <span className="text-red-500">*</span></label>
                <div className="relative">
                    <input type={show ? "text": "password"} id="password" placeholder="Min of 8 characters" className="input w-full" {...register("password")}/>
